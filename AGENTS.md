...
├── src/lib/articles/           # Article store (globalThis __fubaoArticles; single read/write source)
├── src/
...
│   │   ├── rewards/            # Points redemption (server page + client redeem flow)
...
│   │    /api/user/{checkin,points,favorites,history,rewards})
│   └── lib/
│       ├── api/
...
- **Prerender invariant**: `/login` (route group `(auth)`) is a statically prerendered route (no dynamic API), and its client form (`login/client.tsx`) calls `useSearchParams()` at render — so the page MUST keep that component inside a `<Suspense>` boundary (`page.tsx`); removing it breaks `next build` with missing-suspense-with-csr-bailout ("Error occurred prerendering page /login"). `/register` avoids this by reading the referral code via `useEffect`+`localStorage`. `/talisman`/`search-box.tsx` also use `useSearchParams()` but are dynamic pages (server reads searchParams), so they don't need the boundary
...
- Points Redemption (`/rewards`): the spend side of the points economy (check-in/welcome points previously had no sink). Contracts: `GET /api/user/rewards` (session required) → `{data: {points, rewards: Reward[], redemptions: Redemption[]}}`; `POST /api/user/rewards {rewardId}` → 200 `{data: {redemption: {rewardId, rewardTitle, couponCode, pointsSpent, redeemedAt}, pointsRemaining}}` — atomically `spendPoints` (insufficient → 400 "Not enough points") + mints a personal coupon (`PTS-XXXXXX`, ambiguous-free alphabet) via `createPersonalCoupon` + `claimCoupon` so the coupon engine owns issuance. Reward catalog (`lib/points/reward-catalog.ts`): 5 tiers (100–3000 pts) mapping to fixed/percentage/free-shipping coupons. Redemptions persist on globalThis `__fubaoRedemptions` (`lib/points/redemption-store.ts`). `Coupon` carries `isPersonal` — personal PTS- coupons are hidden from the public `/api/coupons` catalog and always `usageLimit: 1, perUserLimit: 1`. Coupon burn path (fixed engine defects): `validateCoupon` no longer treats "claimed" as "used" (previously a claimed coupon could never be applied — used check now uses `usedCount` only); `completeOrder` calls `markCouponUsed` exactly once per order (guarded by `order.promoCounted`), incrementing `usedCount` and burning the user's claim, so a completed single-use coupon is rejected (422) on later carts. Entry points: account Quick Actions ("Redeem Points") + points card link ("Redeem for coupons")
...
- Articles end-to-end (`/admin/ai-training` → `/api/articles` → `/articles`): previously the route kept its own module-level `let articles` array (lost on HMR) while the storefront read a separate static copy — published AI articles NEVER appeared on `/articles`. Now `src/lib/articles/article-store.ts` (globalThis `__fubaoArticles`, seed-once) is the single source: the route AND `lib/api getArticles()/getArticleBySlug()` both read it, so `POST /api/articles` (ai-training publish button, payload title/excerpt/content/category/author/tags) is visible on the storefront immediately. Contracts: GET `?slug=` → single article or 404; default list published-only, `?published=false` → ALL incl drafts; POST requires title/content/category/author (400), category validated against `culture|guide|news|faq` (400), explicit slug or derived-from-title, duplicate slug → 409, `published` defaults true (ai-training publishes immediately), 201 with full article; PUT `{id, ...fields}` updates any field (unknown id → 404); draft → PUT publish → appears on storefront. `Article` type lives in the store module and is re-exported from `@/lib/api`. Sitemap article slugs (`getArticles()`) now include published AI articles automatically
...
Stores already on globalThis: `spree-compat/order-store`, `auth/user-store` (seed-once guard `__fubaoUsersSeeded`), `coupons/coupon-store` (seeds when empty), `distribution/distribution-store`, `giveaways/giveaway-store` (seeds when empty), `notifications/notification-store` (seed-once via `has()` check), `crypto/payment-store` (demo wallet seeded via `has()` check), `blessing/blessing-store`, `wishes/wish-store` (seed-once), `reviews/review-store` (`__fubaoReviews`, seed-once), `articles/article-store` (`__fubaoArticles`, seed-once), `newsletter/newsletter-store`, `points/redemption-store` (`__fubaoRedemptions`, plain array — append-only), `api/user/checkin` route (`__fubaoCheckIns`), `api/user/favorites` route (`__fubaoFavorites`), `api/user/history` route (`__fubaoHistory`), and the legacy orders array in `lib/api/index.ts` (`__fubaoLegacyOrders`). `merchant/merchant-store` is a read-only seed array (no mutation APIs) — safe as-is. When adding a new store, follow the same pattern and seed idempotently (guard with a flag or `size === 0` check so re-instantiation never wipes runtime writes).
...
- **Build-time NODE_ENV trap**: both deploy scripts `set -a; source deploy/fubao.env` (which exports `NODE_ENV=production` for PM2 runtime) BEFORE calling `scripts/build.sh` — and `pnpm install` under `NODE_ENV=production` skips ALL devDependencies (typescript/tsup/tailwindcss are build-time devDeps → `next build` dies at `next.config.ts` transpile with "Cannot find module 'typescript'"). `scripts/build.sh` therefore forces `NODE_ENV=development` on its install line + a `require.resolve('typescript')` guard that fails loudly; `next build` itself is unaffected (it pins production semantics internally). Sandbox builds never hit this because they don't source fubao.env
- Env vars are process-injected only (no dotenv auto-load); JWT_SECRET must be replaced from the dev default — the repo is PUBLIC, so `src/lib/auth/jwt.ts` fails closed: with `NODE_ENV=production` and no `JWT_SECRET` env, signAccessToken/signRefreshToken throw instead of signing with the (now world-known) fallback secret; dev keeps the fallback for DX. A matching FATAL console error is logged at module load. install.sh always injects a random `JWT_SECRET`, so a correctly installed server never triggers this
...
**One-click deploy**: `deploy/install.sh --domain www.fubao.ltd` (installs Node 24/pnpm/PM2/Nginx if missing, clones to `/www/wwwroot/fubao`, generates `deploy/fubao.env` with a strong random JWT_SECRET — gitignored, never overwrite, builds, starts PM2 process `fubao`, writes the BT Nginx vhost `fubao.conf` proxying :80 → 127.0.0.1:5000, attaches BT-issued SSL if present) and `deploy/update.sh` (pull → build → health-check `/api/blessing` → reload; failed health check stops PM2 and prints `git reflog` rollback instead of serving a broken build). Re-running install.sh on an existing clone now ff-only auto-pulls latest code when the branch matches and the working tree is clean (dirty tree → warn + deploy as-is; pull failure → warn + continue) — previously it only fetched and deployed stale code, which stranded servers on pre-fix builds. Production domain is `www.fubao.ltd`; repo `github.com/junnyjchen/fubaoltd` (public — zero-auth clone/pull; both scripts' `git_authed` auth layer stays as an optional path for private forks: `--deploy-key`/`--token`, `GIT_TERMINAL_PROMPT=0`, token persisted to fubao.env for update.sh reuse). History was audited before going public: no `.env`/key/cert files were ever committed (only `token/route.ts` code matched secret-name greps); `deploy/fubao.env` is generated server-side only and gitignored
...
- Product image pipeline (`upload → admin → storefront`): `POST /api/upload` (session-required 401 gate BEFORE multipart parse; accepts png/jpeg/webp, ≤5MB) stores to object storage under `products/{ts}_{rand}.{ext}` and returns `{success, data: {key, url,…}}`. `GET /api/images/{key}` (key must be URL-encoded — `products/…` contains a slash that breaks the single-segment dynamic route) returns `{success, url, key}` with a signed URL, or `{success, placeholder, url}` when object storage isn't configured. Admin product create (`/admin/products`) has a file input → `/api/upload` → `imageKey` (+10×10 preview); `POST/PUT /api/admin/products` accept `imageKey` (must be a `products/` key → 400 otherwise; PUT empty string resets to the default `talisman-{slug}.jpg`). `ProductImage` (`components/shared/product-image.tsx`, client): with `image_key` it fetches the signed URL then renders `<img loading="lazy">` (root carries `data-image-key`); SSR emits the TalismanSVG fallback, `<img>` appears after hydration; no key or fetch failure → SVG motif forever. Consumed on detail page, `/talisman` listing, homepage ProductCard. `createProductAdmin` accepts `imageKey`; every seed product carries `image_key: 'talisman-{slug}.jpg'` (non-existent → SVG fallback by design)
